-- Comprehensive Fix for Profile Visibility
-- This script will ensure all profiles appear on the frontend

-- Step 1: First, let's see what we have
SELECT 'Before Fix - Current State' as info, COUNT(*) as total FROM public.profiles;

-- Step 2: Fix all profiles to be visible
-- Set all profiles as active and published if they have basic info
...
-- Step 3: Fix any remaining NULL values
UPDATE public.profiles 
SET 
  ...
  is_profile_complete = COALESCE(is_profile_complete, TRUE),
...
WHERE 
  ...
  is_profile_complete IS NULL;

-- Step 4: Ensure all profiles have proper default values for required fields
UPDATE public.profiles 
SET 
  bio = COALESCE(bio, ''),
...
-- Step 5: Drop all existing RLS policies and recreate them
...
-- Step 6: Create new comprehensive RLS policies
-- Allow everyone to view published and active profiles
...
-- Step 9: Show sample of visible profiles
SELECT 
  'Sample Visible Teachers' as info,
  id,
  email,
  full_name,
  role,
  ...
  created_at
FROM public.profiles 
WHERE role = 'teacher' AND is_active = TRUE AND is_published = TRUE
ORDER BY created_at DESC
LIMIT 5;

SELECT 
  'Sample Visible Schools' as info,
  id,
  email,
  full_name,
  role,
  ...
  created_at
FROM public.profiles 
WHERE role = 'school' AND is_active = TRUE AND is_published = TRUE
ORDER BY created_at DESC
LIMIT 5;

-- Step 10: Test the exact queries that the frontend will use
SELECT 
  'Frontend Teachers Query Result' as info,
  COUNT(*) as teacher_count
FROM public.profiles 
WHERE role = 'teacher' AND is_published = TRUE AND is_active = TRUE;

SELECT 
  'Frontend Schools Query Result' as info,
